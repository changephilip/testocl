    ...
    __global Assist *d_assist)
{
        ...
        int temp = 0;
        if (binId < numOfBin) {
                gpu_try_assign_kernel(d_bins_start_[binId], d_bins_end_[binId],
                                      binId, d_reads_start_, numOfRead,
                                      d_assist);
        };
        mem_fence();
    ...
    __global Assist *d_assist)
{
        ...
        if (binId < numOfBin) {
                gpu_try_assign_kernel(d_bins_start_[binId], d_bins_end_[binId],
                                      binId, d_ases_start_, numOfASE, d_assist);
                mem_fence();
        }
    ...
    __global struct ASECounter *ACT)
{
        ...
        if (aseId < numOfASE) {
                gpu_try_assign_kernel(d_ases_start_[aseId], d_ases_end_[aseId],
                                      aseId, d_reads_start_, numOfRead,
                                      d_assist);
                mem_fence();
...
#ifdef SE_ANCHOR
                                junctionCount =
                                    d_reads_core[readId].junctionCount;
                                if (junctionCount) {
#pragma unroll
                                        for (int32_t jId = 0;
                                             jId < junctionCount; jId++) {
                                                junction_s =
                                                    d_reads_core[readId]
                                                        .junctions[jId]
                                                        .start_ +
            ...
            __global struct ASECounter * ACT)
        {
                ...
                if (aseId < numOfASE) {
                        ...
                        countIn = act.anchor[0] + act.anchor[1] +
                                  act.anchor[3] / float(act.artRange.end_ -
                                                        act.artRange.start_);
...
#endif
                        d_ase_psi[aseId] = ASEPsi{d_ases_core[aseId].gid_h,

                           ...
                           h_Bins &h_bins, int32_t numOfBin)
{
    ...
    queue.enqueueWriteBuffer(cl_d_bins.start_, CL_TRUE, 0,
                             sizeof(uint64_t) * numOfBin, &(h_bins.start_[0]));
    queue.enqueueWriteBuffer(cl_d_bins.end_, CL_TRUE, 0,
                             sizeof(uint64_t) * numOfBin, &(h_bins.end_[0]));
    queue.enqueueWriteBuffer(cl_d_bins.strand, CL_TRUE, 0,
                             sizeof(uint8_t) * numOfBin, &(h_bins.strand[0]));
    queue.enqueueWriteBuffer(cl_d_bins.core, CL_TRUE, 0,
                             sizeof(bin_core_t) * numOfBin, &(h_bins.core[0]));
    return cl_d_bins;
}
struct cl_d_ASEs {
    cl::Buffer start_;
                           ...
                           h_ASEs &h_ases, int32_t numOfASE)
{
    ...
    queue.enqueueWriteBuffer(cl_d_ases.end_, CL_TRUE, 0,
                             sizeof(uint64_t) * numOfASE, &(h_ases.end_[0]));
    queue.enqueueWriteBuffer(cl_d_ases.strand, CL_TRUE, 0,
                             sizeof(uint8_t) * numOfASE, &(h_ases.strand[0]));
    queue.enqueueWriteBuffer(cl_d_ases.core, CL_TRUE, 0,
                             sizeof(ase_core_t) * numOfASE, &(h_ases.core[0]));
    return cl_d_ases;
}
...
void cl_HandleBin(h_Bins &h_bins, h_Reads &h_reads, h_ASEs &h_ases)
{
    ...
    boost::compute::device bc_gpu = boost::compute::system::default_device();
...
*/
    ...
    boost::compute::vector<uint64_t> bc_d_starts((uint64_t)numOfRead,
                                                 bc_context);
    boost::compute::vector<uint64_t> bc_d_ends((uint64_t)numOfRead, bc_context);
    ...
    */
    ...
    if (boostSort(bc_d_starts, d_indices, bc_queue)) {
        ...
        boost::compute::gather(d_indices.begin(), d_indices.end(),
                               bc_d_ends.begin(), bc_d_ends.begin(), bc_queue);
        boost::compute::gather(d_indices.begin(), d_indices.end(),
    ...
    boost::compute::copy(bc_d_starts.begin(),bc_d_starts.end(),h_reads.start_.begin(),bc_queue);
    boost::compute::copy(bc_d_ends.begin(),bc_d_ends.end(),h_reads.end_.begin(),bc_queue);
    boost::compute::copy(bc_d_strand.begin(),bc_d_strand.end(),h_reads.strand.begin(),bc_queue);
    bc_queue.finish();
    /*
    boost::compute::copy(h_reads.core.begin(), h_reads.core.end(),
                         bc_d_cores.begin(), bc_queue);
    */
    ...
    queue.finish();
    err = queue.enqueueWriteBuffer(cl_d_indices, CL_TRUE, 0,
                                   sizeof(int32_t) * numOfRead, indices);
    checkCLIOBuffer(err, __LINE__);
    err = queue.enqueueWriteBuffer(cl_d_cores_in, CL_TRUE, 0,
                                   sizeof(read_core_t) * numOfRead, &(h_reads.core[0]));
    checkCLIOBuffer(err, __LINE__);
    // cl::Buffer
    // bc_d_cores(CLEnv.context,CL_MEM_READ_WRITE,sizeof(read_core_t)*numOfRead,NULL);
    // gather cores and tranfer to cl_d_reads.core
    autoSetKernelArgs(allKernel.gather_kernel, cl_d_indices, cl_d_cores_in,
                      cl_d_reads.core, (uint64_t)numOfRead);
    ...
    autoSetKernelArgs(allKernel.gpu_assign_read_ASE_kernel, cl_d_ases.start_,
                      cl_d_ases.end_, cl_d_ases.strand, cl_d_ases.core,
                      numOfASE, cl_d_reads.start_, cl_d_reads.end_,
                      cl_d_reads.strand, cl_d_reads.core, numOfRead,
                      d_assist_read_ases, ACT);
    err = queue.enqueueNDRangeKernel(allKernel.gpu_assign_read_ASE_kernel,
                                     offset, global_size_ASE, local_size);
    checkCLEnqueue(err, __LINE__);
    queue.finish();
    // count psi
    /*DONE CL: count PSI*/
    ...
    autoSetKernelArgs(allKernel.gpu_count_PSI, cl_d_ases.start_, cl_d_ases.end_,
                      cl_d_ases.strand, cl_d_ases.core, numOfASE, d_ase_psi,
                      ACT);

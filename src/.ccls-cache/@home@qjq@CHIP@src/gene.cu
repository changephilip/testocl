...
{
        d_Reads d_reads;
        CUDA_SAFE_CALL(cudaMalloc((void **)&(d_reads.start_),
                                  sizeof(uint64_t) * numOfRead));
        CUDA_SAFE_CALL(
            cudaMalloc((void **)&(d_reads.end_), sizeof(uint64_t) * numOfRead));
        CUDA_SAFE_CALL(cudaMalloc((void **)&(d_reads.strand),
                                  sizeof(uint8_t) * numOfRead));
        CUDA_SAFE_CALL(cudaMalloc((void **)&(d_reads.core),
                                  sizeof(read_core_t) * numOfRead));

        CUDA_SAFE_CALL(
                cudaMemcpy(d_reads.start_, h_reads.start_.data(),sizeof(uint64_t) * numOfRead,cudaMemcpyHostToDevice));
...
d_ASEs gpu_chipMallocASE(h_ASEs &h_ases, int32_t numOfASE)
{
        ...
        // numOfASE); std::memcpy(d_ases->end_, h_ases.end_.data(),
        // sizeof(uint64_t) * numOfASE); std::memcpy(d_ases->strand,
        // h_ases.strand.data(), sizeof(uint32_t) * numOfASE);
        // std::memcpy(d_ases->core, h_ases.core.data(), sizeof(ase_core_t) *
        // numOfASE);
...
d_Bins gpu_chipMallocBin(h_Bins &h_bins, int32_t numOfBin)
{
        ...
        // numOfBin); std::memcpy(d_bins->end_, h_bins.end_.data(),
        // sizeof(uint64_t) * numOfBin); std::memcpy(d_bins->strand,
        // h_bins.strand.data(), sizeof(uint32_t) * numOfBin);
        // std::memcpy(d_bins->core, h_bins.core.data(), sizeof(bin_core_t) *
        // numOfBin);
        return d_bins;
}
...
void HandleBin(h_Bins &h_bins, h_Reads &h_reads, h_ASEs &h_ases)
{
        ...
        gpu_assign_read_kernel<<<nBlock, blockSize>>>(
            d_bins, numOfBin, d_reads, numOfRead, d_assist_reads);
        CUDA_SAFE_CALL(cudaDeviceSynchronize());

        float *d_tempTPM, *d_tpmCounter;
        // d_tempTPM
        CUDA_SAFE_CALL(
            cudaMalloc((void **)&d_tempTPM, numOfBin * sizeof(float)));
        CUDA_SAFE_CALL(cudaMemset(d_tempTPM, 0, numOfBin * sizeof(float)));
        // d_tpmCounter
        int32_t tpmSize = (nextPow2(nBlock) + 1) / 2;
        CUDA_SAFE_CALL(
            cudaMalloc((void **)&d_tpmCounter, tpmSize * sizeof(float)));
        ...
        CUDA_SAFE_CALL(cudaMalloc((void **)&d_assist_read_ases,
                                  sizeof(Assist) * numOfASE));
        ASECounter *ACT;
        CUDA_SAFE_CALL(
            cudaMalloc((void **)&ACT, sizeof(ASECounter) * numOfASE));
        // compute number of thread block
